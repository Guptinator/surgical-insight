...
def calc_et_prefill(l5, l4, l3, l2):
    if l3: return ET_PREFILL["L3-S1"]
    if l4 and l5: return ET_PREFILL["L4-S1"]
    if l4 and not l5: return ET_PREFILL["L4-5 only"]
...
# ══════════════════════════════════════════════════════════════════════════════
# TAB 6 — ALIF OPERATIVE RISK CALCULATOR (retrained model v2, AUC 0.710)
# ══════════════════════════════════════════════════════════════════════════════
with tabs[5]:
    st.subheader("🏥 ALIF Operative Risk Calculator")
    st.markdown(
        "Enter patient demographics and **planned operative levels** to estimate "
        "individualised 30-day complication risk. Selecting levels automatically "
        "pre-fills a typical exposure time estimate based on registry data. "
    ...
    with col_inp:
        st.markdown("**Patient demographics**")
        rc_age  = st.slider("Age (years)", 18, 90, 60, key="rc_age")
        rc_sex  = st.selectbox("Sex", ["Male","Female"], key="rc_sex")
        rc_bmi  = st.slider("BMI (kg/m²)", 15.0, 55.0, 29.0, 0.5, key="rc_bmi")
        rc_asa  = st.selectbox("ASA class", [1,2,3], index=1, key="rc_asa")
        rc_abd  = st.selectbox("Prior abdominal surgery", ["No","Yes"], key="rc_abd")
        ...
        rc_l2 = st.checkbox("L2-3",  value=False, key="rc_l2")

        # Auto-compute ET pre-fill from level selection
        l5_v = 1 if rc_l5 else 0
        l4_v = 1 if rc_l4 else 0
        l3_v = 1 if rc_l3 else 0
        l2_v = 1 if rc_l2 else 0
        et_default = int(calc_et_prefill(l5_v, l4_v, l3_v, l2_v))

        st.markdown("**Procedural parameters**")
        rc_et = st.slider(
            f"Estimated exposure time (min) — pre-filled from level selection",
            8, 80, et_default, key="rc_et"
        )
        rc_st = st.slider("Anticipated surgical time (min)", 40, 240, 90, key="rc_st")

        # Level summary note
        n_levels = l5_v + l4_v + l3_v + l2_v
        level_names = [l for l, v in [("L5-S1",l5_v),("L4-5",l4_v),("L3-4",l3_v),("L2-3",l2_v)] if v]
        if level_names:
            st.info(f"**Planned exposure:** {' + '.join(level_names)} "
                    f"({'single' if n_levels==1 else str(n_levels)+'-level'}) · "
                    f"Pre-filled ET: **{et_default} min**")
        else:
            st.warning("Please select at least one operative level.")

    # ── Compute risk ──────────────────────────────────────────────────────────
    sex_b = 1 if rc_sex == "Male" else 0
    abd_b = 1 if rc_abd == "Yes"  else 0
    rev_b = 1 if rc_rev == "Yes"  else 0

    risk = alif_risk_v2(rc_age, sex_b, rc_bmi, rc_asa, abd_b, rev_b,
    ...
    with col_out:
        # Risk score card
        st.markdown(f"""
        <div style='border-radius:12px;padding:24px 28px;text-align:center;
                    background:{risk_bg};border:2px solid {risk_tc}40;margin-bottom:16px;'>
            <div style='font-size:3.8rem;font-weight:700;color:{risk_tc};'>{risk_pct}%</div>
            <div style='font-size:1.15rem;font-weight:600;color:{risk_tc};margin:6px 0;'>{risk_label}</div>
            <div style='font-size:0.9rem;color:#444441;margin-top:8px;'>{advice}</div>
        </div>""", unsafe_allow_html=True)

        # Risk curve by ET for selected patient
        et_range = np.arange(8, 81, 2)
        risk_curve = [
            round(alif_risk_v2(rc_age, sex_b, rc_bmi, rc_asa, abd_b, rev_b,
                               float(e), float(rc_st), l5_v, l4_v, l3_v, l2_v) * 100, 1)
            for e in et_range
        ]
        fig_rc = go.Figure()
        ...
        fig_rc.add_hline(y=risk_pct, line_dash="dot",
                         line_color="#2E6DB4", line_width=1.8,
                         annotation_text=f"Current: {risk_pct}%",
                         annotation_font_color="#2E6DB4")
        fig_rc.add_vline(x=rc_et, line_dash="dot",
                         line_color="#888780", line_width=1.2)
        fig_rc.update_layout(
            height=240,
            xaxis_title="Exposure time (min)",
            yaxis_title="30-day complication probability (%)",
            yaxis_range=[0, 100],
            showlegend=False,
            plot_bgcolor="white", paper_bgcolor="white",
        ...
        st.caption("Curve shows how risk changes with exposure time for this patient profile. "
                   "Other inputs held constant.")

        # Driver breakdown table
        st.markdown("**Risk driver summary**")
        drivers = []
        if rc_age >= 70:   drivers.append(("Age ≥70",       "🔴 High"))
        elif rc_age >= 60: drivers.append(("Age 60–69",      "🟡 Moderate"))
        else:              drivers.append(("Age <60",        "🟢 Low"))

        if rc_asa == 3:    drivers.append(("ASA III",        "🔴 High"))
        elif rc_asa == 2:  drivers.append(("ASA II",         "🟡 Moderate"))
        else:              drivers.append(("ASA I",          "🟢 Low"))

        if rc_st > 120:    drivers.append(("Surgical time",  "🔴 >2 hours"))
        elif rc_st > 90:   drivers.append(("Surgical time",  "🟡 90–120 min"))
        else:              drivers.append(("Surgical time",  "🟢 <90 min"))

        if n_levels >= 2:  drivers.append(("Levels",         f"🟡 {n_levels}-level"))
        else:              drivers.append(("Levels",         "🟢 Single-level"))

        if rc_abd == "Yes": drivers.append(("Prior abd surgery", "🟡 Present"))
        else:               drivers.append(("Prior abd surgery", "🟢 None"))

        drv_df = pd.DataFrame(drivers, columns=["Factor","Status"])
        st.dataframe(drv_df, hide_index=True, use_container_width=True, height=220)

    # ── Complications by level reference table ────────────────────────────────
    st.markdown("---")
    st.markdown("**Registry data: complication rates by operative level (n=331)**")
    st.caption("Based on the 331-patient registry used to train this model. "
               "Small cell sizes at L3-4 and L2-3 limit interpretation at those levels.")

    level_table = pd.DataFrame([
        {"Operative level": "L5-S1 only (n=239)",    "Overall rate": "15.5%", "ER revisit": "12.1%", "Transfusion": "2.9%",  "Vascular injury": "1.3%", "p vs L5-S1 only": "reference"},
        {"Operative level": "L4-5 only (n=32)",      "Overall rate": "6.2%",  "ER revisit": "3.1%",  "Transfusion": "3.1%",  "Vascular injury": "0.0%", "p vs L5-S1 only": "0.280"},
        {"Operative level": "L4-S1 2-level (n=48)",  "Overall rate": "27.1%", "ER revisit": "10.4%", "Transfusion": "10.4%", "Vascular injury": "4.2%", "p vs L5-S1 only": "0.062"},
        {"Operative level": "L3-S1 3-level (n=4)",   "Overall rate": "0.0%",  "ER revisit": "0.0%",  "Transfusion": "0.0%",  "Vascular injury": "0.0%", "p vs L5-S1 only": "1.000 ⚠"},
    ])
    st.dataframe(level_table, hide_index=True, use_container_width=True)
    st.caption("⚠ Very small n — interpret with caution. p-values from Fisher exact test. "
               "Transfusion rate notably higher in 2-level (L4-S1) vs single-level cases (10.4% vs 2.9%, p=0.049).")

    st.markdown("---")
    st.caption(
        "⚠️ **Clinical decision support only.** This tool is based on a single-centre "
